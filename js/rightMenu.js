    ...
    if (document.body.clientWidth > 768) {
        ...
        if (a.test(window.getSelection().toString()) || el.tagName == "A") {
            rm.openWithNewTab = function () {
                ...
                var url = el.href ? el.href : window.getSelection().toString()

                window.open(reg.test(url) ? url : `https://${url}`);
            }
            rm.copyLink = function () {
                let url = el.href ? el.href : window.getSelection().toString()
...
$("#menu-copyArticleLink").on("click", function () {
    removeRightMenu();
    copyArticleLink()
})
...
$("#menu-openWithNewTab").on('click', function () {
    removeRightMenu();
    rm.openWithNewTab()
})
...
$("#menu-saveImg").on('click', function() {
    removeRightMenu()
    rm.saveImg()
})